"""Достаёт дополнительные поля публикации (абстракт, DOI, редакторы, обложку)
из JSONB-колонки `raw` и подкладывает их в ответ API.

Парсинг происходит при каждом чтении публикации — БД не меняем.
Чистая функция: на входе raw dict, на выходе обогащённый PublicationOut.
"""
...
from app.schemas import AuthorRef, PublicationOut
...
def enrich_publication(base: PublicationOut, raw: dict[str, Any] | None) -> PublicationOut:
    """Возвращает новый PublicationOut с дополнительными полями из raw JSONB.

    Не мутирует `base`. Если `raw` пустой/None — возвращает базу как есть.
    """
    if not raw:
        return base
